...
OUT=os.path.join(paths.RESULTS,'v6')+os.sep
FIG=os.path.join(paths.ROOT,'figures')+os.sep; os.makedirs(FIG,exist_ok=True)
T=pd.read_csv(OUT+'table1_ladders_v6.csv')
F=json.load(open(OUT+'table9_function_decomp_v6.json'))
...
fig.savefig(FIG+'fig1_skill_vs_function_v6.png'); fig.savefig(FIG+'fig1_skill_vs_function_v6.pdf'); plt.close(fig)
...
fig.savefig(FIG+'fig2_ladder_v6.png'); fig.savefig(FIG+'fig2_ladder_v6.pdf'); plt.close(fig)

# ---- Fig 3 : 2x2 (귀속 그림을 대체) ----
fig,ax=plt.subplots(figsize=(6.2,3.4))
xm={'1e-4':0,'1e-5':1,'0':2}
# 비제약
ax.plot([0,2],[nat.ratio,natnd.ratio],'o-',color=C1,lw=1.8,ms=7,label='unconstrained readout')
# 정합 — 적합 관문을 통과한 점만 실선으로 잇는다
ax.plot([1,2],[mid.ratio,ali.ratio],'o-',color=C2,lw=1.8,ms=7,label='aligned readout')
ax.plot([0,1],[a1.ratio,mid.ratio],ls=':',color=C2,lw=1.2)
ax.plot([0],[a1.ratio],'o',mfc='white',mec=C2,mew=1.8,ms=9)
for x,y,c in ((0,nat.ratio,C1),(2,natnd.ratio,C1),(1,mid.ratio,C2),(2,ali.ratio,C2)):
    ax.text(x,y+.10,f'{y:.2f}',ha='center',fontsize=8,fontweight='bold',color='k')
ax.text(-0.16,a1.ratio,f'{a1.ratio:.2f}',ha='right',va='center',fontsize=8,color='#c0392b')
ax.annotate('latent tables collapsed;\nfit gate failed',xy=(0.06,a1.ratio+.04),
...
fig.savefig(FIG+'fig3_two_by_two_v6.png'); fig.savefig(FIG+'fig3_two_by_two_v6.pdf'); plt.close(fig)
...
fig.savefig(FIG+'fig4_collapse_v6.png'); fig.savefig(FIG+'fig4_collapse_v6.pdf'); plt.close(fig)
print('v6 그림 4점 작성')
