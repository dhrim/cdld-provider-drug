...
CK = CD + 'ck_a/'; os.makedirs(CK, exist_ok=True)
CKN = CD + 'ck/'
...
def unit(d, D_cache, arm, fold, kind):
    ...
    torch.manual_seed(2000 + fold * 10 + {'B': 1, 'C': 2}[kind])
    m = CA.AlignedFinder(sizes, ncov, int(d['n_nurse']), int(d['n_drug']), kind)
    t0 = time.time()
    CC.fit(m, D, tr, te, cycles=(3 if kind == 'C' else 6),
           sub_epochs=1, bs=8192, seed=42 + fold, cyclic=(kind == 'C'))
    p = CC.predict(m, D, te)
    res = dict(tag=TAG, model='CDLD-A', arm=arm, fold=int(fold), kind=kind,
               n_te=int(len(te)),
    ...
    print(f'  ✓ CDLD-A {key}  BSS {1-res["sse"]/res["sstot"]:+.5f}  '
