...
CK = CD + 'ck_a2/'; os.makedirs(CK, exist_ok=True)
CKN = CD + 'ck/'
ARMS = ['shuffle2']
TAG = 'base'
...
def unit(d, D_cache, arm, fold, kind):
    ...
    m = CA.AlignedFinder(sizes, ncov, int(d['n_nurse']), int(d['n_drug']), kind)
    t0 = time.time()
    ...
    print(f'  ✓ A2n3 {key}  BSS {1-res["sse"]/res["sstot"]:+.5f}  '
...
def main():
    ...
    for arm in ARMS:
        for f in range(5):
            for ext in ('json',):
                s0, d0 = CD+f'ck_a/{TAG}_{arm}_f{f}_B.{ext}', CK+f'{TAG}_{arm}_f{f}_B.{ext}'
