...
CK = CD + 'ck_a2w/'; os.makedirs(CK, exist_ok=True)
CKN = CD + 'ck/'
...
def unit(d, D_cache, arm, fold, kind):
    ...
    torch.manual_seed(3000 + fold * 10 + {'B': 1, 'C': 2}[kind])
    m = CA.AlignedFinder(sizes, ncov, int(d['n_nurse']), int(d['n_drug']), kind, n_ward=int(d['n_ward']))
    t0 = time.time()
    CC.fit(m, D, tr, te, cycles=(3 if kind == 'C' else 6),
           sub_epochs=1, bs=8192, seed=42 + fold, cyclic=(kind == 'C'), no_decay=('u_tab', 'v2_tab'))
    p = CC.predict(m, D, te)
    res = dict(tag=TAG, model='CDLD-A2', arm=arm, fold=int(fold), kind=kind,
               n_te=int(len(te)),
    ...
    print(f'  ✓ CDLD-A2 {key}  BSS {1-res["sse"]/res["sstot"]:+.5f}  '
...
def main():
    ...
    for arm in ARMS:
        for f in range(5):
            for ext in ('json',):
                s0, d0 = CD+f'ck_a2w_src/{TAG}_{arm}_f{f}_B.{ext}', CK+f'{TAG}_{arm}_f{f}_B.{ext}'
                if os.path.exists(s0) and not os.path.exists(d0): shutil.copy(s0, d0)
            s0, d0 = CD+f'ck_a/pred_{TAG}_{arm}_f{f}_B.npz', CK+f'pred_{TAG}_{arm}_f{f}_B.npz'
            if os.path.exists(s0) and not os.path.exists(d0): shutil.copy(s0, d0)
    queue = [(arm, f, k) for k in ('B','C') for arm in ARMS for f in range(5)]
    t0 = time.time(); done = 0
