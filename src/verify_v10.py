"""v10 검증 — v9 와 같은 수치 검사에, 본문/부록 «위치» 검사를 더한다.
원 docstring: — 본문 수치를 겹 단위 결과에서 재계산하고 불일치 시 비영 종료."""
...
MSS=_p('투고본_한글_v10.md'); SUP=_p('보충자료_한글_v10.md')
if MSS:
    _raw=open(MSS).read()
    ...
    필수_본문=[('확증 실행 전에 고정했다',1),
        ...
        # ── v10 임상 독자 문안 — 본문에 있어야 한다
        ('원 분석 계획이 적은 「입원\n단위로 섞는다」와는 보존하는 성질이 다르다',1),
        ('사후에 추가한\n분석',1),
        ('무작위 배정 20회의 중앙값을 전부 넘는다',1),
        ('사람마다 숫자 하나',3),
        ('투약 기록을 «입력한» 사람이지 투약을 «수행한» 사람이 아니다',1),
        ('임상적으로\n부적절한 누락이 아니다',1),
        ('실시간 예측이 아니라',1),
        ('무작위 배정 3회 기준',1),
        ('100건 중 39건 → 40건',1),
        ('세 문장',1),
        ('cyclic dual latent discovery',1),('CDLD',8),('순환 이중 잠재\n발견',1)]
    필수_전체=[('product of provider and drug',1),('weighted Pearson r',1),
        ('이 시정은 동결 계획에 근거가 없다',1),
        ('귀무 2회',2),
        ('교환 상대가 없',1),('실제로» 동일한 입원',1),
        ('진단용 복제 계열',1),
        ('64차원 학습 잠재 표',1),('estimator family alone',1),
    ...
    _main_plain=re.sub(r'^>.*$','',t,flags=re.M)          # 영문 인용 블록 제외
    _ban_main=['estimand','읽기층','잠재 표','잠재 벡터','64차원','ULD','ILD','Δ_main','Δ_latent',
               'Δ_total','κ =','γ\*','frac_inter','sd(D)/sd(M)','RQ1','RQ2','RQ3','RQ4',
               '이식성','Q2b','U0 ','저랭크 이중선형 판','순환 이중 잠재 판','추정기 계열']
    for wd in _ban_main:
        c=_main_plain.count(wd); ok&=(c==0); n+=1
        print(f'{"OK " if c==0 else "FAIL"}  본문 금칙(임상 독자) 「{wd}」 {c}건')
    # 세 경계 — 쉽게 쓰려다 넘지 않았는가
    _main_noref=_main_plain.split('# 참고문헌')[0]          # 참고문헌 제목은 인용이다
    for wd in ['간호사가','간호사는','간호사의','투약 누락','medication omission','medication error','전향 예측이다','실시간 예측이다']:
        c=_main_noref.count(wd); ok&=(c==0); n+=1
        print(f'{"OK " if c==0 else "FAIL"}  경계 위반 「{wd}」 {c}건')
    ...
    _bad_in=['증거가 아니다','저자가 번호를','실행 전에 정의함','쓰지 않는 방법',
             '전문은 보충자료','작업본','예시이며',
             'estimand','읽기층','잠재','RQ','Δ','이식','추정기','사다리']
