"""CDLD-A 판정 — 사전 문서 §9 의 귀속 분해 규칙을 «그대로» 실행한다.
...
★ native 와 CDLD-A 를 «같은 귀무 팔»(shuffle0, shuffle1)로 맞춰 비교한다.
...
def main():
    nat = load('ck'); ali = load('ck_a')
    n_ali = len([k for k in ali if k[2] in 'BC'])
    print(f'CDLD-A 완료 단위 {n_ali}/30 (B,C x 3팔 x 5겹)\n')
    ...
    r_A = L_ali['ratio']
    ident = l + gamma + kappa
    print(f'\ngamma*  = {gamma:+.5f}   (native B - CDLD-A B, 귀무 보정)')
    ...
    print(f'  C 단계 BSS  native {L_nat["C"]:.5f}  CDLD-A {L_ali["C"]:.5f}  '
    ...
    json.dump(dict(native3=L_nat3, native2=L_nat, aligned=L_ali,
              ...
              open(CD + 'diag/adj_a.json', 'w'), indent=1)
