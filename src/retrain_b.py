...
def main(fold):
    ...
    torch.manual_seed(1000 + fold)                      # 진단 복제용 고정 시드
    m = CC.Finder(sizes, ncov, n_nurse=int(d['n_nurse']),
                  n_drug=int(d['n_drug']), drug_latent=True)
    t0 = time.time()
    CC.fit(m, D, tr, te, cycles=6, sub_epochs=1, bs=8192, seed=42 + fold, cyclic=False)
    p = CC.predict(m, D, te)
    sse = float(((y[te] - p) ** 2).sum())
    sstot = float(((y[te] - y[tr].mean()) ** 2).sum())
    bss = 1 - sse / sstot
    orig = json.load(open(CK + f'base_real_f{fold}_B.json'))
    ...
    json.dump(rec, open(CKB + f'B_f{fold}.json', 'w'))
