...
CK = CD + 'ck_a2mid/'; os.makedirs(CK, exist_ok=True)
CKN = CD + 'ck/'
...
def unit(d, D_cache, arm, fold, kind):
    ...
    CC.fit(m, D, tr, te, cycles=(3 if kind == 'C' else 6),
           sub_epochs=1, bs=8192, seed=42 + fold, cyclic=(kind == 'C'), no_decay=(), latent_wd=1e-5)
    p = CC.predict(m, D, te)
    res = dict(tag=TAG, model='CDLD-A2-mid', arm=arm, fold=int(fold), kind=kind,
               n_te=int(len(te)),
    ...
    print(f'  ✓ A2mid {key}  BSS {1-res["sse"]/res["sstot"]:+.5f}  '
...
def main():
    ...
    for arm in ARMS:
        for f in range(5):
            for ext in ('json',):
                s0, d0 = CD+f'ck_a2/{TAG}_{arm}_f{f}_B.{ext}', CK+f'{TAG}_{arm}_f{f}_B.{ext}'
                if os.path.exists(s0) and not os.path.exists(d0): shutil.copy(s0, d0)
            s0, d0 = CD+f'ck_a2/pred_{TAG}_{arm}_f{f}_B.npz', CK+f'pred_{TAG}_{arm}_f{f}_B.npz'
            if os.path.exists(s0) and not os.path.exists(d0): shutil.copy(s0, d0)
